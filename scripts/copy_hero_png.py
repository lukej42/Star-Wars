...
def copy_hero(category: str, slug: str, source: Path) -> bool:
    if not source.is_file():
        return False
    assets = DEFAULT_ASSETS / f"{slug}.png"
    ...
    shutil.copy2(source, image_dir / f"{slug}.png")
    shutil.copy2(source, image_dir / f"{slug}-scene.png")
    return True
...
def main() -> None:
    ...
    parser.add_argument("source", nargs="?", help="Source PNG path (defaults to assets/{slug}.png)")
    args = parser.parse_args()
    source = Path(args.source) if args.source else DEFAULT_ASSETS / f"{args.slug}.png"
