#!/usr/bin/env python3
"""Install timeline cinematic hero PNGs from Cursor assets into wwwroot."""
...
def install_slug(slug: str, source_dir: Path) -> bool:
    ...
    if not src.is_file():
        return False
    image_dir = ROOT / "wwwroot" / "images" / "timelines"
...
def main() -> None:
    ...
    for entry in entries:
        ...
        if install_slug(slug, source):
            ...
            missing.append(slug)
    print(f"Installed {installed}/{len(entries)} timeline hero PNGs from {source}")
    if missing:
