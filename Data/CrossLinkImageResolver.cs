...
public static class CrossLinkImageResolver
{
    ...
    public static string? Resolve(string route)
    {
        ...
        {
            "characters" when segments.Length >= 2 => $"/images/characters/{segments[1]}-scene.webp",
            "jedi" when segments.Length >= 2 => $"/images/jedi/{segments[1]}-scene.webp",
            "sith" when segments.Length >= 2 => $"/images/sith/{segments[1]}-scene.webp",
            "ships" when segments.Length >= 2 => $"/images/ships/{segments[1]}-scene.webp",
            "species" when segments.Length >= 2 => $"/images/species/{segments[1]}-scene.webp",
            "factions" when segments.Length >= 2 => $"/images/factions/{segments[1]}-scene.webp",
            "settlements" when segments.Length >= 2 => $"/images/settlements/{segments[1]}-scene.webp",
            "bounty-hunters" when segments.Length >= 2 => $"/images/bounty-hunters/{segments[1]}-scene.webp",
            "droids" when segments.Length >= 2 => $"/images/droids/{segments[1]}-scene.webp",
            "force-powers" when segments.Length >= 2 => $"/images/force-powers/{segments[1]}-scene.webp",
